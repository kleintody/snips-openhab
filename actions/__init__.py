...
def generate_item_up_down_result_sentence(devices, command):
    l_devices = list(devices)

    if command == "UP":
        command_spoken = "hoch gemacht"
    elif command == "DOWN":
        command_spoken = "runter gemacht"
    else:
        command_spoken = ""
...
def item_up_down_callback(assistant, intent_message, conf):
    ...
    command = "UP" if intent_message.intent.intent_name == user_intent("moveItemUp") else "DOWN"

    if devices is None:
        return False, UNKNOWN_DEVICE.format("hoch machen" if command == "OPEN" else "runter machen")
...
def add_callbacks(assistant):
    ...
    assistant.add_callback(user_intent("moveItemUp"), item_up_down_callback)
    assistant.add_callback(user_intent("moveItemDown"), item_up_down_callback)
